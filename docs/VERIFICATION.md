...
- 访客统计解析器：12 项测试、39 个断言通过，覆盖分页、异常响应、非法国家代码与错误 JSON 结构。
- 站点输出：13 项检查通过，包括首页结构、文章阅读次数、统计脚本、地图资源、旧地址跳转和敏感信息扫描。
- HTMLProofer：40 个 HTML 文件、135 条内部链接、10 个文件中的内部锚点检查通过；检查包括 Images、Links、Scripts，禁用外部链接探测。
- 浏览器：首页显示个人介绍和访客统计，不显示文章卡片；文章仍可通过搜索与归档访问。
- 数学公式和 Mermaid 流程图实际渲染；浅色与深色显示正常。
- 390 × 844 手机视口：首页没有横向滚动，地图自动缩放；测试后已恢复桌面视口。

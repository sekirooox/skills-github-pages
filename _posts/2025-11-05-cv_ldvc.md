...
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/4bac0af65d004e63bf213986c10049df.png){: referrerpolicy="no-referrer" }
## 动机
+ 之前的工作仍然有对不可见类的过拟合
+ 这归咎于解码器的不合理设计：解码器使用文本嵌入进行查询，不适合零样本任务。**因为视觉空间相比文本并不足够结构化用于分割。**
+ 让文本嵌入对齐视觉嵌入，**会造成较大的嵌入空间迁移，损害分割性能**。
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/4f97be97ff784c7ca7081d8fa6c9f1b2.png){: referrerpolicy="no-referrer" }

![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/6e471f90b80c4da9bbb3e0873c3980ca.png){: referrerpolicy="no-referrer" }
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/d0a31e1139d14be7b218a5738d41b856.png){: referrerpolicy="no-referrer" }
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/3a65aa6ade48461d9499ea2937fdd527.png){: referrerpolicy="no-referrer" }

## 文本适配器
+ **将关系描述符表达为文本适配器**，与ZegCLIP**完全一致**，毫无区别

![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/40ff27e17c4845c6a23b2d7b6c493967.png){: referrerpolicy="no-referrer" }

## 视觉共识解码器

### 总体流程
+ $V_j$是**视觉编码器中的某一层输出**，~~作者好像没有具体给出j的取值？~~ 
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/f0618df063ac45a0bc659e6d457cd7e4.png){: referrerpolicy="no-referrer" }
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/ee845d2457d0422692b58c80c09d2846.png){: referrerpolicy="no-referrer" }

## 视觉共识自注意力
+ 基于路由注意力的机制
+ 简单来说就是关注图片中相关的区域，例如“草”的区域关注“草”的区域。
+ 首先要对**图像窗口化**，然后**计算注意力权重**，**保留权重高的窗口**，还原得到高度相关的$K_s$和$V_s$(只有部分patch)
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/a8bdb6dccb044e2ab8dd96949a66f014.png){: referrerpolicy="no-referrer" }

## 掩码产生机制
+ 掩码产生机制几乎完全一样
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/bba85d01ae8a4855b83b52c09a949779.png){: referrerpolicy="no-referrer" }
...
+ 在转导设置下，**几乎没有任何有效提升**。
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/83ff5bc7161d4af29c481d0a6088dd50.png){: referrerpolicy="no-referrer" }
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/61acf22ab2ff43b2956e13189631037c.png){: referrerpolicy="no-referrer" }


## 消融实验
+ 交叉注意力图可以学习一下。
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/be2da4574a80437ca2ce670aa680ebdd.png){: referrerpolicy="no-referrer" }

![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/033581c21f554c2aa490832cfd8409f4.png){: referrerpolicy="no-referrer" }
...
[
\text{Att}_i = \text{Softmax}\left(\frac{Q_i K_{s,i}^\top}{\sqrt{d}} \right) V_{s,i}
]

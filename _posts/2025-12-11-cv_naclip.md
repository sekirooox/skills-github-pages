...
math: true
description: "本文整理“NACLIP”涉及的模型原理、关键方法与实践要点，便于理解和复习相关技术。"
---
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/b2c673cd7956430cbdef1211a3547cfa.png){: referrerpolicy="no-referrer" }
# 方法
提出了一个高斯核，对于(i,j)的patch，对其注意力分数进行修正，**靠近该位置的分数奖励高，远离的分数奖励低**，与SCCLIP的KK自注意力进行叠加。
然后最后一层也是移除残差连接和FFN

![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/a69d7848d1a140d3bd22ec17a677f89b.png){: referrerpolicy="no-referrer" }
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/fafc8841bd3e46149761df97f878eff8.png){: referrerpolicy="no-referrer" }
实验发现，即使只有高斯核，完全忽略KK注意力分数，表现也比KK自注意力分数好，这说明**CLIP确实缺乏这种能力**。

![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/bb6b540b4abe43b2acc2990fb1fd1d76.png){: referrerpolicy="no-referrer" }
...
* 对于位置 $$(i,j)$$ 的 patch，计算与所有位置 $$(m,n)$$ 的相似度：
  $$\text{sim}_{ij,mn} = \frac{q_{ij}^\top k_{mn}}{\sqrt{d}}$$

* softmax 得到注意力权重，做 value 的加权和：
  $$
  \alpha_{ij,mn} = \text{softmax}_{m,n}(\text{sim}_{ij,mn}),\\
  A_{ij} = \sum_{m,n} \alpha_{ij,mn} v_{mn}
  $$

* 最后线性变换：
  $$\text{SA}(Z)_{ij} = A_{ij} W_o$$
...
* 对最后一层的 patch 表示 $$Z^{(L)}_{ij}\in\mathbb{R}^{D}$$ 和所有文本向量做 cosine 相似：
  ...
  $$
  s_{ij}^{(c)} = \frac{\langle Z^{(L)}_{ij}, T_c\rangle}
  {\lVert Z^{(L)}_{ij}\rVert\lVert T_c\rVert},\quad c=1,\dots,C
  $$
...
2. **Key-Key 相似度相似性模块（KK-Sim）**
   ...
   $$
   \text{sim}_{ij,mn} = \frac{k_{ij}^\top k_{mn}}{\sqrt{d}}
   $$
...
3. **邻域 + 语义联合注意力（A#）**
   ...
   $$
   A^{\#}_{ij} = \text{softmax}\big(\frac{k_{ij}^\top k_{mn}}{\sqrt{d}}
...
离散到 patch 网格上（$$m=1\dots h,\quad n=1\dots w$$）：
...
$$
A_{ij} = \text{softmax}\big(\vartheta((i,j);\varepsilon)\big)
$$
...
$$
A^{\#}_{ij} = \text{softmax}\Big(
\frac{k_{ij}^\top k_{mn}}{\sqrt{d}} + \vartheta((i,j);\varepsilon)_{mn}
\Big)
$$

* logits = “语义相似度 + 空间邻域偏置”；
* 输出 patch 表示：
  $$\tilde{Z}_{ij} = A^{\#}_{ij} W_o \in \mathbb{R}^D$$

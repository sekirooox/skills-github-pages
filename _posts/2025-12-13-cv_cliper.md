...
description: "本文整理“CLIPer”涉及的模型原理、关键方法与实践要点，便于理解和复习相关技术。"
---

# CLIPer
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/5a72c7eecf9e4d81aed3b34477e934bb.png){: referrerpolicy="no-referrer" }


# 动机
提示CLIP的关键在于空间特征表示的改进上。
可以利用早期的特征和注意力来改进，而不是依赖最后一层注意力或者其他VFM模型。
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/c542ffecdf734ceea2242bce72269fdf.png){: referrerpolicy="no-referrer" }


扩散模型对于**局部细节的空间关系表示比较好**，可以用于进行语义分割。

![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/86cae48553b34a06a5616814a97cb28b.png){: referrerpolicy="no-referrer" }


# 方法
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/61ac56d857b14faeb88073a6896c93d1.png){: referrerpolicy="no-referrer" }

## 早期层融合
就是把早期层的注意力图进行平均化处理，**替代最后一层的注意力图**。
**最后一层的FFN和残差连接进行移除**。

![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/dbd248d14b1848fabdded90773d4c6d8.png){: referrerpolicy="no-referrer" }
作者还加入了一个**中间特征融合**，**不止限于中间注意力图融合**

![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/c8681c1d0dc14bfd9d2e72ed203cb2fc.png){: referrerpolicy="no-referrer" }
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/20899aff4ad048efa514f364d237c935.png){: referrerpolicy="no-referrer" }
消融实验发现使用矩阵乘法效果最好。
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/a62b562239de4068955f02c8d0fed685.png){: referrerpolicy="no-referrer" }
